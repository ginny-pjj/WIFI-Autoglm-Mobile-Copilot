...
class ActionHandler:
    ...
    def set_step_context(
        self, *, task_text: str | None = None, step: int | None = None
    ) -> None:
        """Optional hook used by patched agent.py on cloud; safe no-op by default."""
    ...
    def execute(
        ...
        return self.execute_action(action, screen_width, screen_height)
    ...
    def execute_action(
        ...
        try:
            ...
            return ActionResult(
                success=False,
                should_finish=action_name in {"Take_over", "Interact"},
    ...
    def _handle_takeover(self, action: dict, width: int, height: int) -> ActionResult:
        ...
        soft_markers = ("截图", "截屏", "screenshot", "隐私", "敏感屏幕", "blocked", "黑屏", "锁屏")
        is_screenshot_takeover = any(marker in lower_msg for marker in soft_markers)

        if os.getenv("PHONE_AGENT_CONTINUE_ON_SCREENSHOT_TAKEOVER", "true").lower() in (
            "1",
            "true",
            "yes",
        ):
            if is_screenshot_takeover:
                time.sleep(1.5)
                return ActionResult(True, False, f"忽略疑似截图误判并继续观察：{message}")

        # Cloud/subprocess runs have no TTY; never block on input().
        if not sys.stdin.isatty():
            time.sleep(1.5)
            return ActionResult(True, False, f"非交互环境自动跳过接管：{message}")

        if os.getenv("PHONE_AGENT_FAIL_ON_TAKEOVER", "true").lower() in ("1", "true", "yes"):
            return ActionResult(False, True, f"需要人工接管：{message}")
