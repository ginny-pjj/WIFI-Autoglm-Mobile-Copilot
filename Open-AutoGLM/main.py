...
def _skip_keyboard_check() -> bool:
    return os.getenv("PHONE_AGENT_SKIP_KEYBOARD_CHECK", "").lower() in (
        "1",
        "true",
        "yes",
    )
...
def check_system_requirements(
    ...
    if device_type == DeviceType.ADB:
        ...
        try:
            ...
            if _skip_keyboard_check():
                print("⚠️ SKIPPED (remote ADB slow; ensure ADB Keyboard is enabled)")
            else:
                print("❌ FAILED")
                print("   Error: ADB command timed out.")
                print(
                    "   Remote fix: set PHONE_AGENT_ADB_CMD_TIMEOUT=45 or "
                    "PHONE_AGENT_SKIP_KEYBOARD_CHECK=true in server/.env.cloud"
                )
                all_passed = False
        except Exception as e:
            print("❌ FAILED")
...
def main():
    ...
    if args.task:
        ...
        print(f"\nResult: {result}")
        if result.startswith("Action failed:") or result.startswith("需要人工接管：") or result.startswith("Model error:"):
            sys.exit(2)
    else:
        # Interactive mode
